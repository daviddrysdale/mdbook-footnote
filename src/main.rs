...
lazy_static! {
    static ref FOOTNOTE_RE: Regex = Regex::new(r"(?s)\{\{footnote:\s*(?P<content>.*?)\}\}").unwrap();
}
    ...
    fn run(&self, _ctx: &PreprocessorContext, mut book: Book) -> Result<Book, Error> {
        book.for_each_mut(|item| {
            if let mdbook::book::BookItem::Chapter(chap) = item {
                ...
                chap.content = FOOTNOTE_RE.replace_all(&chap.content, |caps: &regex::Captures| {
                    let content = caps.name("content").unwrap().as_str().to_owned();
                    footnotes.push(content);
                    let idx = footnotes.len();
                    format!("<sup><a name=\"to-footnote-{}\">[{}](#footnote-{})</a></sup>", idx, idx, idx)
                }).to_string();

                if !footnotes.is_empty() {
                    chap.content += "<p><hr/>\n";
                    for (idx, content) in footnotes.into_iter().enumerate() {
                        chap.content += &format!("\n\n<a name=\"footnote-{}\">[{}](#to-footnote-{})</a>: {}", idx+1, idx+1, idx+1, content);
                    }
                }
            }
        });
